...
D21 = 1.045e08;       % Decoupler gain

%% PART 2: Haalman Tuning Parameter Calculations
K = 0.0002392; Tau = 785; L = 20;

% Analytic formulas for Haalman tuning technique
Kp_Hl = Tau / (2 * K * L);
Ti_Hl = Tau;
Td_Hl = L / 2;
...
[t, x] = ode45(@tank_mimo_haalman_core, tspan, x0);

%% PART 4: Plotting the Haalman Performance
figure('Name', 'MIMO Tank - Haalman Evaluation', 'NumberTitle', 'off');
...
grid on;
title('Nonlinear Temperature Dynamics Under Haalman PID Tuning');
xlabel('Time (sec)'); ylabel('Temperature (\circC)');
legend('Haalman PID', 'Reference SP');

%% PART 5: Plant Core Dynamics Function
function dxdt = tank_mimo_haalman_core(t, x)
    global A_tank kv Tin_nominal inv_rho_Cp h0 T0
    global Kp_level Ki_level Kp_temp Ki_temp D21
    global Kp_Hl Ki_Hl Kd_Hl
    ...
    % Feedback control law utilizing Haalman parameters
    der_err_T = -dT_dt_approx;
    delta_u2_fb = Kp_Hl * err_T + Ki_Hl * int_err_T + Kd_Hl * der_err_T;
